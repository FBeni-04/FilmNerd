  ...
  export default function MovieDetail({ slug, movieId }) {
    ...
    useEffect(() => {
      let alive = true;
      if (!user || !id) {setIsWatchlist(false); return; }
      (async () => {
        try {
          const r = await fetch(`${API_BASE}/watchlist/exists/?movie_id=${encodeURIComponent(id)}`, {
            headers: { Authorization: `Bearer ${access}` },
          });
          const j = r.ok ? await r.json() : { exists: false };
    ...
    async function toggleWatchlist() {
      ...
      try {
        if (isWatchlist) {
          ...
          if (res.ok || res.status === 400 || res.status === 409) setIsWatchlist(true);
        }
      } finally {
                  ...
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                    ...
                    {/* Kedvenc gomb */}
                      ...
                      <button
                        ...
                        className={`inline-flex items-center gap-2 rounded-full border px-3 py-1.5 text-sm transition
                            ${isWatchlist ? "border-emerald-500/40 bg-emerald-500/10 text-emerald-300 hover:bg-emerald-500/20" 
                                          : "border-white/15 bg-white/5 text-neutral-200 hover:bg-white/10"}`}
                        title={isWatchlist ? "Remove from Watchlist" : "Add to Watchlist"}
                      >
                      {isWatchlist ? (
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 576 512" className="h-6 w-6 fill-green-400">
                                <path d="M288 32c-80.8 0-145.5 36.8-192.6 80.6-46.8 43.5-78.1 95.4-93 131.1-3.3 7.9-3.3 16.7 0 24.6 14.9 35.7 46.2 87.7 93 131.1 47.1 43.7 111.8 80.6 192.6 80.6s145.5-36.8 192.6-80.6c46.8-43.5 78.1-95.4 93-131.1 3.3-7.9 3.3-16.7 0-24.6-14.9-35.7-46.2-87.7-93-131.1-47.1-43.7-111.8-80.6-192.6-80.6zM144 256a144 144 0 1 1 288 0 144 144 0 1 1 -288 0zm144-64c0 35.3-28.7 64-64 64-11.5 0-22.3-3-31.7-8.4-1 10.9-.1 22.1 2.9 33.2 13.7 51.2 66.4 81.6 117.6 67.9s81.6-66.4 67.9-117.6c-12.2-45.7-55.5-74.8-101.1-70.8 5.3 9.3 8.4 20.1 8.4 31.7z"/>
                            </svg>
                        ) : (
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 576 512" className="h-6 w-6 fill-white">
