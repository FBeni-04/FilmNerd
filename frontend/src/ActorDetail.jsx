...
export default function ActorDetail({ slug, personId }) {
  ...
  // 1) elsődleges: route param (/actor/:id)
  ...
  useEffect(() => {
    ...
    async function load() {
      if (!id) {
        setError("Hibás vagy hiányzó színész azonosító.");
        setLoading(false);
        return;
      }
      try {
  ...
  // Top filmek, csak színészi (Acting) szerepek, popularity szerint
  const topActingCredits = useMemo(() => {
    let cast = data?.combined_credits?.cast || [];
    ...
    // Opcionális: min. szavazatszám szűrés
    cast = cast.filter((c) => (c.vote_count || 0) >= 50);
    ...
    return cast
        ...
        if (bv !== av) return bv - av;                     // 1) Legjobb értékelés
        const ac = a.vote_count || 0;
        const bc = b.vote_count || 0;
        if (bc !== ac) return bc - ac;                     // 2) Több szavazat előrébb
        return (b.popularity || 0) - (a.popularity || 0);  // 3) Tie-breaker
        })
        .slice(0, 12);
    }, [data]);
